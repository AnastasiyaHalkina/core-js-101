...
function findFirstSingleChar(str) {
  for (let i = 0; i < str.length; i += 1) {
    const nextPos = str.indexOf(str[i], i + 1);
    const prevPos = str.indexOf(str[i]);
    if (nextPos === -1 && prevPos === i) return str[i];
  }
  return null;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (['[', '(', '{', '<'].includes(str[i])) {
      stack.push(str[i]);
    } else if (str[i] in brackets && stack.length >= 1) {
      const end = stack.pop();
      if (brackets[str[i]] !== end) {
        return false;
      }
